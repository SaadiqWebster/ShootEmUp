...
engine = e.Engine((256,384),'Shoot-em-Up', 2)
plain_text = f.Font(engine.get_tileset('font','plain'),1)
...
def MainMenu():
    ...
    while Run:
        ...
        for i in range(len(options)):
            font_color = (255,255,0) if menuselect == i else (255,255,255)
            engine.game_draw_to_screen(plain_text.draw(options[i], font_color), ((engine.CAMERA_SIZE[0]/2), (engine.CAMERA_SIZE[1]/2)+y_offset), 'center')
            y_offset += 10
...
def ShipSelect():
    ...
    while Run:
        ...
        engine.game_draw_to_screen(plain_text.draw('Player 1',(255,255,255)), (engine.CAMERA_SIZE[0]*1/4, (engine.CAMERA_SIZE[1]/2)-10), 'center')
        engine.game_draw_to_screen(plain_text.draw('Player 2',(255,255,255)), (engine.CAMERA_SIZE[0]*3/4, (engine.CAMERA_SIZE[1]/2)-10), 'center')
        
        for i in range(1,engine.MAX_PLAYERS+1):
            x_offset = 0 if i == 1 else engine.CAMERA_SIZE[0]/2
            if i in engine.player_list:
                engine.game_draw_to_screen(plain_text.draw('Ready!', (255,255,0)), (engine.CAMERA_SIZE[0]*1/4+x_offset, (engine.CAMERA_SIZE[1]/2)+5), 'center')
            else:
                engine.game_draw_to_screen(plain_text.draw('Press Any Button', (255,255,255)), (engine.CAMERA_SIZE[0]*1/4+x_offset, (engine.CAMERA_SIZE[1]/2)+5), 'center')
    ...
    for player_id in engine.player_list:
        if len(engine.player_list) < 2:
            ...
            x_offset = 7/10
        engine.add_object('player_ships', p.PlayerShip((engine.CAMERA_SIZE[0]*x_offset)-10, engine.CAMERA_SIZE[1], player_id, (engine.CAMERA_SIZE[0],engine.CAMERA_SIZE[1]-32), engine.animation_database))
...
def Gameplay(level):
    ...
    background = b.Background(engine.CAMERA_SIZE,engine.tileset_database['background'])
    hud = h.Hud(engine.CAMERA_SIZE, plain_text)
